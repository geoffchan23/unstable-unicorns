...
export function describeNeighWindow(stack: StackItem[], me: PlayerId, n: NeighNames): NeighWindowText {
  ...
  let title: string;
  if (neighCount === 0) {
    title = `${n.player(base.player)} plays ${baseName}`;
  } else {
    ...
    title = `${n.player(top.player)} plays ${n.card(top.card)} on ${target}`;
  }
  ...
  const sub = `${outcomeNow} ${outcomeIfNeigh}`;

  const banner = neighCount === 0
    ? 'waiting for Neighs'
    : `${neighCount} Neigh${neighCount > 1 ? 's' : ''} on it · ${standsIfPassed ? 'it stands so far' : "it's Neigh'd so far"}`;

  return { title, sub, featured: top.card, base: base.card, neighCount, standsIfPassed, banner };
}
