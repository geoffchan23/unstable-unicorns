...
async function step(page) {
  for (const [sel, pick] of [
    ['[data-testid=prompt]', (l) => l.locator('button:enabled').first()],
    ['[data-testid=neigh]', (l) => l.getByRole('button', { name: 'Let it happen' })],
    ['[data-testid=target]', (l) => l.locator('.choice').first()],
    ['[data-testid=detail]', (l) => l.locator('[data-testid=play], button:has-text("Cancel")').first()],
...
for (const [tag, device] of [['pixel', devices['Pixel 7']], ['ipad', devices['iPad (gen 7)']]]) {
  for (const scheme of ['light', 'dark']) {
    ...
    { const c = page.getByTestId('hand').locator('button.card').first(); if (await c.count()) { await c.click(); await page.waitForTimeout(300); await shot('3b-detail'); await page.getByTestId('detail').getByRole('button', { name: 'Cancel' }).click(); } }
